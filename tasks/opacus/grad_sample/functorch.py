...
def make_functional(mod: nn.Module, disable_autograd_tracking: bool = False):
    ...
    def fmodel(new_params_values, *args, **kwargs):
        ...
        first_param_type = next(iter(new_params_dict.values())).dtype # new insert - POORNA
        args = tuple([
            value.to(first_param_type) if isinstance(value, torch.Tensor) and value.dtype.is_floating_point else value
            for value in args
        ]) # new insert - POORNA
